...
% pre-compute curvature distribution and normal vectors
...
% refine at the trailing edge if necessary
...
% now generate coordinates for the C-grid
  y=  spline( s,x, 0 );                     % position
  g=  spline( z,t, max(l(2),z(1)) );        % normal vector
  for i=1:n
      v(1,1,i)= y(1)+g(1)*r(i);
      v(2,1,i)= y(2)+g(2)*r(i);
  end
  ...
  for j=2:m-1  
     y=  spline( s,x, l(j) );                         % position
     g=  spline( z,t, min(max(l(j),z(1)),z(nt)) );    % normal vector
     for i=1:n
        v(1,j,i)= y(1)+g(1)*r(i);
        v(2,j,i)= y(2)+g(2)*r(i);
     end
  ...
  y=  spline( s,x, 1 );                     % position
  g=  spline( z,t, min(l(m-1),z(nt)) );     % normal vector
  for i=1:n
     v(1,m,i)= y(1)+g(1)*r(i);
     v(2,m,i)= y(2)+g(2)*r(i);
  end
